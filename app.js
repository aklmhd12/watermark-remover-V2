/*
 * Authorized PDF Watermark Cleanup — v2
...
(() => {
    ...
    targets: [
      {
        ...
        terms: ['tamilguru.lk', 'www.tamilguru.lk', 'more past papers at tamilguru.lk'],
        hexSignatures: [
  ...
  function targetFromHexOperand(hex) {
    ...
    if (bytes.length % 2 === 0) {
      ...
      const direct = codes.map((c) => c >= 32 && c < 127 ? String.fromCharCode(c) : '?').join('');
